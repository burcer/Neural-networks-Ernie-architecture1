...
def affine_sigmoid_forward(x, w, b,noise=0):
  """
...
def affine_pwlsig_backward(dout, cache):
  """
  Backward pass for the affine-relu convenience layer
  """
  fc_cache, sigmoid_cache = cache
  da = pwlsig_backward(dout, sigmoid_cache)
  dx, dw, db = affine_backward(da, fc_cache)
  return dx, dw, db

def affine_pwlsig2_backward(dout, cache):
  """
  Backward pass for the affine-relu convenience layer
  """
  fc_cache, sigmoid_cache = cache
  da = pwlsig2_backward(dout, sigmoid_cache)
  dx, dw, db = affine_backward(da, fc_cache)
  return dx, dw, db
...
def affine_tanh_backward(dout, cache):
  ...
  """
  fc_cache, sigmoid_cache = cache
  da = tanh_backward(dout, sigmoid_cache)
  dx, dw, db = affine_backward(da, fc_cache)
  return dx, dw, db
...
def conv_sigmoid_forward(x, w, b, conv_param):
  """
  A convenience layer that performs a convolution followed by a ReLU.
...
def conv_sigmoid_backward(dout, cache):
  """
  Backward pass for the conv-relu convenience layer.
  """
  conv_cache, relu_cache = cache
...
def conv_sigmoid_pool_forward(x, w, b, conv_param, pool_param):
  """
  Convenience layer that performs a convolution, a ReLU, and a pool.
...
def conv_sigmoid_pool_backward(dout, cache):
  """
  Backward pass for the conv-relu-pool convenience layer
  """
  conv_cache, relu_cache, pool_cache = cache
  ds = max_pool_backward_fast(dout, pool_cache)
  da = sigmoid_backward(ds, relu_cache)
  dx, dw, db = conv_backward_fast(da, conv_cache)
  return dx, dw, db
...
def conv_bn_relu_forward(x, w, b, gamma, beta, conv_param, bn_param):
  """
  Convenience layer that performs a convolution, a spatial batch normalization, a ReLU.
