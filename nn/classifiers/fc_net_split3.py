...
class ThreeLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim1=512,hidden_dim2=256, num_classes=10,conv_size=256,
    ...
    self.params['W2_1'] = weight_scale * np.random.randn(hidden_dim1/2, hidden_dim2/2)
    self.params['W2_2'] = weight_scale * np.random.randn(hidden_dim1/2, hidden_dim2/2)

    self.params['b2_1'] = np.zeros(hidden_dim2/2)
    self.params['b2_2'] = np.zeros(hidden_dim2/2)
  ...
  def loss(self, X, y=None,noise=0,noise2=0,test=0,parallel_samples_output=1):
    ...
    W2_1 = self.params['W2_1']
    W2_2 = self.params['W2_2']
    W3 = self.params['W3']
    ...
    if noise ==1:
      ...
      randsel=10000*np.random.rand(10000,512)+1
      randint=np.array(np.floor(randsel))-1
      hidden = data[indices.astype(int),randint.astype(int)]
      hidden1 = hidden[:,0:128]
      hidden2 = hidden[:,128:256]
      hidden3 = hidden[:,256:384]
      hidden4 = hidden[:,384:512]     
...
##next layer:
    hidden_cache2_1 = []
    hidden_cache2_2 = []
    out1, tmp1 = affine_forward(np.hstack([hidden1,hidden2]), self.params['W2_1'], self.params['b2_1'])
    out2, tmp2 = affine_forward(np.hstack([hidden3,hidden4]), self.params['W2_2'], self.params['b2_2'])

    out=np.hstack([out1,out2])

    hidden_cache2_1.append(tmp1)
    hidden_cache2_2.append(tmp2)
    ...
    if noise ==0:
      ...
      if self.activation ==2:
        hidden, tmp = pwlsig_forward(out) #change tos 
      if self.activation ==3:
        hidden2_1, tmp1 = tanh_forward(out1) #change to 
        hidden2_2, tmp2 = tanh_forward(out2) #change to 

      if self.activation ==4:
        hidden, tmp = pwlsig2_forward(out) #change to 

      hidden2=np.hstack([hidden2_1,hidden2_2])
    ...
    if self.activation ==3:
      dx1, dW2_1, db2_1 = affine_tanh_backward(dhidden[:,0:128], hidden_cache2_1)   #change to affine_sigmoid later
      dx2, dW2_2, db2_2 = affine_tanh_backward(dhidden[:,128:256], hidden_cache2_2)  
    ...
    if self.activation ==3:
      dx11, dW1_1, db1_1 = affine_tanh_backward(dx1[:,0:128], hidden_cache1)   #change to affine_sigmoid later
      dx22, dW1_2, db1_2 = affine_tanh_backward(dx1[:,128:256], hidden_cache2)  
      dx33, dW1_3, db1_3 = affine_tanh_backward(dx2[:,0:128], hidden_cache3)  
      dx44, dW1_4, db1_4 = affine_tanh_backward(dx2[:,128:256], hidden_cache4)        
    ...
    reg = self.reg
    loss += 0.5 * reg * (np.sum(W1_1*W1_1) + np.sum(W2_1*W2_1)+np.sum(W2_2*W2_2) +np.sum(W1_2*W1_2)+np.sum(W1_3*W1_3)+np.sum(W1_4*W1_4) +np.sum(W3*W3) )
    dW1_1 += reg * W1_1
    dW1_2 += reg * W1_1
    dW1_3 += reg * W1_1
    dW1_4 += reg * W1_1
